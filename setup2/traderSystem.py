...
def runStrategy(df, dailyDf, Money, stopLoss, stdMax, stdMin):
    ...
    for day in range(0,size):
        ...
        lowPred = df.iloc[day].pred_low
        highPred = df.iloc[day].pred_high
        diff = highPred-lowPred
        relativeMargin = abs(highPred-lowPred)/lowPred
        
        # if(relativeMargin < 0.06):
        #     continue
        
        
        # correção dos valores preditos
        #lowPred = lowPred+(stdMin*diff)
        #highPred = highPred-(stdMax*diff)
        
        ## todo: descomente depois
        lowPred = lowPred*(1+stdMin)
        highPred = highPred*(1-stdMax)
        
        ## todo: apagar depois
        # lowPred = lowPred*1.01
        # highPred = highPred*0.99
